// details.js（病原体ピル非表示・CreatorsURLそのまま）

const $ = (s)=>document.querySelector(s);

function getParam(name){ const u=new URL(location.href); return u.searchParams.get(name)||''; }
function getAllParams(){ const u=new URL(location.href); const p={}; ['q','fi','fj','fd','ft','fg'].forEach(k=>p[k]=u.searchParams.get(k)||''); return p; }
...
function scoreRelated(item, basis){
  let score=0, shared={topics:[],infection:false,journal:false};
...
function renderRelated(current, indexItems, params){
  const basis={topics:new Set(current.topics||[]), infection_type:current.infection_type||'', journal:current.journal||''};
  const scored=[];
  for (const it of indexItems){
    if (String(it.slug)===String(current.slug)) continue;
    const {score}=scoreRelated(it,basis);
    if (score<=0) continue;
...
(async function main(){
  ...
  const terms = [...new Set([...(params.q||'').split(/\s+/).filter(Boolean), params.fi, params.fj, params.fd, params.ft, params.fg].filter(Boolean))];
  ...
  try{
    ...
    $('#detail').innerHTML = `
      ...
      ${(topics||tags) ? `<div class="detail-pills">${topics}${tags}</div>` : ''}
